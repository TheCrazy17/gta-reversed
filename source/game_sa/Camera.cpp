...
void CCamera::HandleCameraMotionForDuckingDuringAim(CPed* ped, CVector* source, CVector* targPosn, bool arg5) {
    ...
    auto factor = 0.0f; // default: no change
    if (task && ped->bIsDucking && !task->m_bIsAborting) {
        factor = -0.35f;
    }

    if (!arg5) {
        m_fDuckAimCamMotionFactor += CTimer::GetTimeStep() * 0.13f * (factor - m_fDuckAimCamMotionFactor);
    }
...
void CCamera::CalculateDerivedValues(bool bForMirror, bool bOriented) {
    ...
    const auto mag = std::sqrt(m_fCamFrontXNorm * m_fCamFrontXNorm + m_fCamFrontYNorm * m_fCamFrontYNorm);
    if (mag == 0.0f) {
        m_fCamFrontXNorm = 1.0f;
    } else {
        const auto invMag = 1.0f / mag;
        m_fCamFrontXNorm *= invMag;
        m_fCamFrontYNorm *= invMag;
    }
}
