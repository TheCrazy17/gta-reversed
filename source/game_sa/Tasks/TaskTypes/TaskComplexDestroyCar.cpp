...
CTask* CTaskComplexDestroyCar::ControlSubTask(CPed* ped) {
    if (!m_VehicleToDestroy || m_VehicleToDestroy->m_fHealth <= 0.0f) {
        return nullptr;
    }
    if (m_arg0) {
    ...
    eTaskType newSubTaskType;
    switch (m_pSubTask->GetTaskType()) {
    case TASK_COMPLEX_DESTROY_CAR_MELEE:
        if (ped->GetActiveWeapon().IsTypeMelee()) {
            return m_pSubTask;
        }
        newSubTaskType = TASK_COMPLEX_DESTROY_CAR_ARMED;
        break;
    case TASK_COMPLEX_DESTROY_CAR_ARMED: {
        if (!ped->IsCurrentlyUnarmed() || ped->IsPlayer()) {
            return m_pSubTask;
        }

        auto slot = 0u;
        for (; slot < NUM_WEAPON_SLOTS; slot++) {
            if (ped->m_aWeapons[slot].m_Type > WEAPON_UNARMED) {
                ped->SetCurrentWeapon((int32)slot);
                break;
            }
        }
        if (slot != NUM_WEAPON_SLOTS) {
            return m_pSubTask; // Found & equipped a weapon - stay in armed mode
        }

        ped->SetCurrentWeapon(0);
        newSubTaskType = TASK_COMPLEX_DESTROY_CAR_MELEE;
        break;
    }
    default:
        return m_pSubTask;
    ...
    return m_pSubTask->MakeAbortable(ped, ABORT_PRIORITY_URGENT)
        ? CreateSubTask(newSubTaskType, ped)
        : m_pSubTask;
}
